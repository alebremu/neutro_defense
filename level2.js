...
function mkNeutrophil(x,y) {
  return { x, y, patrolX:x, patrolDir:1,
    load:0, digestTimer:0, phRange:PHAGO_RANGE,
    hovered:false, flashT:0, bounceVx:0, bounceT:0,
...
function tick() {
  ...
  towers.forEach(t => {
    // Patrol
    if (t.bounceVx!==0) { t.x+=t.bounceVx; t.bounceVx*=0.8; if(Math.abs(t.bounceVx)<0.05)t.bounceVx=0; }
    else { t.x+=t.patrolDir*0.5; if(t.x>t.patrolX+PATROL_R||t.x<t.patrolX-PATROL_R)t.patrolDir*=-1; }
    ...
    enemies.filter(e=>e.type==='hypha'&&!e.dead).forEach(h=>{
      ...
      if(Math.hypot(t.x-cx,t.y-h.y)<18){t.bounceVx=(t.x<h.x+h.len/2?-1:1)*3.5;t.bounceT=12;t.patrolX=t.x;h.hitT=6;if(Math.random()<0.18)showTip('Frustrated phagocytosis','The hypha is too large to engulf — the neutrophil bounces off. This triggers extracellular degranulation!');}
    });
